...
fun main() {
    ...
    // If vy is non-positive, there won't be a peak
    val vyMin = 1

    // If vy is greater than the target end, the first step after crossing y=0 will overshoot
    val vyMax = -targetY1
        ...
        for (vy in vyMin..vyMax) {
            for (t in tx1..tx2) {
                ...
                if (y >= targetY1 && y <= targetY2) {
                    highestPeak = max(highestPeak, (vy * vy + vy) / 2)
                    break
                }
            }
        }
